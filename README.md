...
Feature extraction needs `ffmpeg` on PATH.

## Datasets

Neither dataset is redistributable; request access from the original authors.

- **VGAF** (Sharma et al., IEEE TAFFC 2023). We use the official train (2,661 clips) and validation (766 clips) splits. The test set is not public.
- **GECV** (Quach et al., Pattern Recognition 2022). 408 of the 627 clips are publicly available. Evaluated with stratified 10-fold cross-validation (`fold_seed: 42`).

Set the paths in `configs/vgaf.yaml` and `configs/gecv.yaml`, which have `/path/to/datasets/...` placeholders, or override per run:
...
All backbones used in this project are frozen.
...
`<cache_root>/<modality>/<clip_id>.h5` and skips existing files.
...
VGAF trains five seeds and reports on the official validation set.
...
These isolate why structured aggregators beat holistic pooling on VGAF.

```bash
python -m models.ger_classifier_b   # prints "identical" for all published cells
```

Two configs: `mean_pool_uniform` replaces the learned temporal attention of
...
`scripts/evaluate.py` reproduces the reported numbers from checkpoints without retraining:
